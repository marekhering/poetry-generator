...
class LSTMModel:
    ...
    def create_with_attention(self, total_words: int, output_dim, max_sequence_len: int, rnn_units: int = 256):
        ...
        x = Embedding(total_words, output_dim, input_length=max_sequence_len - 1)(model_input)
        x = LSTM(rnn_units, return_sequences=True)(x)
        x = Attention(units=total_words)(x)
...
if __name__ == '__main__':
    LSTMModel().create_with_attention(9000, 2048, 15).model.summary()
    LSTMModel().create(9000, 2048, 15).model.summary()
